...
{
    [Serializable]
    public class NewCriAtomMixerBehaviour : PlayableBehaviour
    {
        ...
        public override void ProcessFrame (Playable playable, FrameData info, object playerData)
        {
            ...
            {
                ScriptPlayable<NewCriAtomBehaviour> inputPlayable  = (ScriptPlayable<NewCriAtomBehaviour>)playable.GetInput (inputPort);
                NewCriAtomBehaviour                 clipBehaviour  = inputPlayable.GetBehaviour ();
                NewCriAtomClip                      newCriAtomClip = clip.asset as NewCriAtomClip;
                float                               inputWeight    = newCriAtomClip.ignoreBlend ? 1f : playable.GetInputWeight (inputPort);
                ...
                if (clipBehaviour != null)
                {
                    if (m_StopOnWrapping && wrapped)
                    {
                        ...
                        {
                            clipBehaviour.Stop (newCriAtomClip.stopWithoutRelease);
                        }
                    }
                    ...
                    else if (newCriAtomClip.muted == false)
                    {
                        long seekTimeMs       = (long)((m_Director.time - clip.start) * 1000.0);
                        bool isDirectorPaused = (m_Director.state == PlayState.Paused);

                        var playConfig = new NewCriAtomClipPlayConfig (
                            newCriAtomClip.cueSheet,
                            newCriAtomClip.cueName,
                            newCriAtomClip.acbPath,
                            newCriAtomClip.awbPath,
                            newCriAtomClip.acfPath,
                            seekTimeMs,
                            clip.timeScale,
                            newCriAtomClip.loopWithinClip
                        );

                        if (clipBehaviour.IsClipPlaying == false)
                        {
                            /* Entering clip for the first time */
                            if (IsEditor == false)
                            {
                                m_Bind = clipBehaviour.tempObject.GetComponent<CriAtomSource> ();
                            ...
                            newCriAtomClip.SetClipDuration (clipBehaviour.CueLength);
                        }
                        else
                        {
                            var now = DateTime.Now;
                            if (IsEditor == true && isDirectorPaused &&
                                now - m_lastScrubTime > new TimeSpan (0, 0, 0, 0, cScratchTimeIntervalMs))
                            {
                                /* Scrubing the track */
